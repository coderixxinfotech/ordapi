...
while [ $retries -lt 3 ]; do
    ...
    ord --rpc-url bitcoin-container:8332 --bitcoin-rpc-user mempool --bitcoin-rpc-pass mempool --data-dir /root/.local/share/ord/$runDir server --http-port 8080 &>/dev/stdout &
...
while true; do
    ...
    then
        ...
        ord --rpc-url bitcoin-container:8332 --bitcoin-rpc-user mempool --bitcoin-rpc-pass mempool --wallet ord wallet balance
        echo "Checking wallet balance using index.redb not being used as server..."
        ord --rpc-url bitcoin-container:8332 --bitcoin-rpc-user mempool --bitcoin-rpc-pass mempool --data-dir /root/.local/share/ord/$copyDir --wallet ord wallet balance
    fi
    sleep 1800
done